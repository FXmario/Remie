...
- **Slash commands** — type `/` to open an anchored command menu for `/memories`, `/chats`, `/connect`, and `/models`. The first match is highlighted automatically; use Up/Down, Tab, Enter, hover, or click to choose. Commands are handled locally rather than sent to the model.
