...
- **Agent memory** — the agent can persist durable facts, decisions, and open tasks to `.remie/memory/<name>.md` with the `memory` tool (add/read/clear/list). Named memories live side by side; press `Ctrl+M` to pick which memory is active (the active one is auto-loaded into the system prompt each launch, others stay reachable via `memory(action="read", name="...")`). When a long session nears the context window, dropped messages are summarized into a compact memory note instead of being silently truncated
...
| `Ctrl+M`  | Open memory picker (switch active memory) |
