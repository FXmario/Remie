...
- **Agent memory** — the agent can persist durable facts, decisions, and open tasks to named memory files (`.remie/memory/`) with the `memory` tool (add/read/clear/list/delete). Each memory has a stable UUID id and a display name; press `Ctrl+O` to open the picker, switch the active memory, or delete one (with a confirm). The active memory is auto-loaded into the system prompt each launch; others stay reachable via `memory(action="read", name="...")`. When a long session nears the context window, dropped messages are summarized into a compact memory note instead of being silently truncated. When the agent saves a note without naming a memory, Remie derives the name from the current task (the latest prompt) and makes that memory active, so each task gets its own named memory.
