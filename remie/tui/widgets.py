...
class ModelBadge(Label):
    ...
    def update_config(self, config) -> None:
        ...
        self._context_limit = get_model_context_limit(config.model, provider)
        self._show(self._input_tokens, self._output_tokens)

    def set_context(self, tokens: int, limit: int | None = None) -> None:
        """Show current context occupancy, retaining a discovered model limit."""
    ...
    def _show(self, input_tokens: int = 0, output_tokens: int = 0) -> None:
        ...
        if self._context_tokens:
            context = _format_tokens(self._context_tokens)
            if self._context_limit:
                percent = min(999, round(self._context_tokens * 100 / self._context_limit))
                text += (
                    f" · CTX {context}/{_format_tokens(self._context_limit)}"
                    f" ({percent}%)"
                )
            else:
                text += f" · CTX {context}/?"
